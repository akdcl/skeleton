package 
{
	...
	public class Example07 extends Sprite {
		...
		private function init():void {
			...
			setTimeout(delayEncode, 200, new RobotBiped());
		}
...
class StarlingGame extends Sprite {
	...
	private function updateMovement():void {
		...
		if (moveDir == 0) {
			speedX = 0;
			armature.animation.playTo("stand", 40, 0.2, true, 2);
		}else {
			if (moveDir * face > 0) {
				speedX = 8* face;
				armature.animation.playTo("run", 24, 0.3, true, 2);
			}else {
				speedX = -5 * face;
				armature.animation.playTo("runBack", 32, 0.1, true, 2);
			}
		}
	}
